...
def extract_cad_entities(
    ...
    try:
        ...
        type_suffix = f"_{'_'.join(entity_types)}" if entity_types else ""
        filename = f"entities{layer_suffix}{type_suffix}_{timestamp}.json"

        entities_file = output_dir / filename
...
def calculate_cad_measurements(
    ...
    Returns:
        ...
        - success: bool
        - data: {
            measurements_file: str,  # 测量数据文件路径
            summary: dict            # 汇总统计
          }
        - error: str
    """
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        measurements_file = output_dir / f"measurements_{calculation_type}_{timestamp}.json"
